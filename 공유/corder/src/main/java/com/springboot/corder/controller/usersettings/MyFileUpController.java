...
public class MyFileUpController {
    ...
    @GetMapping("/images/{ofname}")
    @ResponseBody
    public org.springframework.core.io.Resource downloadImage(@PathVariable("ofname") String ofname, Model model)
            throws IOException{
        Member member = memberFileService.getMemberImage(ofname);
